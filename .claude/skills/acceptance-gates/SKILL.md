...
- [ ] **README 里的每条命令当下可跑**——npm 未发布期间不许出现 `npx geo-doctor`（此刻的临时状态；发布后按 [release-versioning-control](../release-versioning-control/SKILL.md) 改回）。检验法：把命令粘进新终端跑一遍
...
- `fetcher.ts` 与 `site.ts` 单测覆盖为 0 是**显式共识**（纯网络层，由真实站点冒烟兜底）：不要为凑数给它们写 mock 单测，也不要引用它们的 0% 来给自己的新代码免测——豁免只此两个文件
