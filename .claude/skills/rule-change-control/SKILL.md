...
- 首日真实案例：`list-table-density` 曾设 200 被 fixture 卡成 `na`（详见 [failure-archaeology](../failure-archaeology/SKILL.md) 案 2），修复决策是**统一内容规则适用门槛为 150**——这个共识没有写在任何代码注释里，改一处必须改全部
- **辨析：仓库里有三种"150"，只有第一种受本共识管**——① 上述 4 条内容规则的**适用门槛**（na 线）；② `access.js-dependency` 的空壳判 0 分线（`units < 150`）；③ `chunk.paragraph-size` 的超长段落线（`LONG_PARAGRAPH_UNITS = 150`）。②③是独立语义，数值巧合，不在"改一处必须全改"范围内
...
4. `docs/methodology.md` 的规则表是**手工维护**的，与代码没有自动同步——改规则名/权重/证据必须手动同步该表（规则表生成脚本在 [diagnostic-commands](../diagnostic-commands/SKILL.md) 的"文档同步"节）
5. 双语文案按 [bilingual-docs-contract](../bilingual-docs-contract/SKILL.md) 检查
6. **安全项**：新规则若把站点来源的字符串拼进 finding，必须先过 `stripControlChars`（[security-boundaries](../security-boundaries/SKILL.md) D2 防线）
7. 跑 `pnpm test:coverage`（CI 不守覆盖率门，见 [acceptance-gates](../acceptance-gates/SKILL.md)）
