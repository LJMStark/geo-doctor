...
description: GEODoctor 个别真实站点的审计怪象排查手册——403 拒绝、SPA 空壳误判、乱码、超时、sitemap 找不到时使用（多数站点都异常则去 debug-audit-pipeline）。含 openai.com 案例与不许伪装 UA 的红线。
---
...
- **误报**：极简落地页（正文本来就 < 150 units）+ 大 bundle 会被当空壳。注意这个 150 是 js-dependency 的**判 0 分线**，与内容规则的 150 **适用门槛**（na 线）语义不同，只是数值巧合（辨析见 [rule-change-control](../rule-change-control/SKILL.md)）。核对方法：`curl -s <url> | grep -c "关键正文词"`，HTML 里真有正文就是误报，考虑给该规则提 issue 而不是现场改阈值（改阈值走 [rule-change-control](../rule-change-control/SKILL.md)）
...
`resolveSitemap` 是**二选一，不是两处都查**：robots.txt 有 Sitemap 声明 → 只试声明的前 2 条，**失败不回退** `/sitemap.xml`；无声明 → 才试 `/sitemap.xml`。推论：站点 robots 声明了一条坏 Sitemap（404 或被 SSRF 过滤），哪怕 `/sitemap.xml` 完好也会判"无 sitemap"。另**不递归 sitemapindex、不猜 /sitemap_index.xml**——以上均为 v0.1 已知局限。另外 robots 声明的 sitemap URL 要过 `safePublicUrl` SSRF 过滤，指向内网/元数据地址的声明会被静默丢弃（安全设计，见 [security-boundaries](../security-boundaries/SKILL.md)）。
