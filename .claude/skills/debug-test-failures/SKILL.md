...
### 2. fixture 阈值敏感（余量仅 25 词）

`good-article.html` main 正文 ≈ 175 units；多条规则适用门槛 = 150。高危操作及其症状：

| 操作 | 症状 |
|---|---|
| fixture 删句子 | `cit.*`/`chunk.list-table-density` 从 pass 变 **na**，"good article passes" 断言挂 |
| 规则门槛调高 | 同上 |
...
- 覆盖率细节：`pnpm test:coverage`。**fetcher.ts / site.ts 单测覆盖率为 0 是显式共识**（网络层由真实站点冒烟兜底,见 [acceptance-gates](../acceptance-gates/SKILL.md)）——不要试图为这两个文件 mock 出单测来"修覆盖率"
